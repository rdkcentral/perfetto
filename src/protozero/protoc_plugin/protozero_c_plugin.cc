 ...
 */
    ...
    for (const std::string& imp : imports) {
      // Try primary; fall through to external for imports under a different
      // source root.
  ...
  // Optional secondary (strip, add) prefix pair, tried when the primary
  // doesn't match. Lets one generation pass cover multiple source roots
  // (e.g. protos/perfetto/ and protos/third_party/). See
  // rfcs/0017-out-of-tree-protos.md.
