  ...
  const tabs: TabsTab[] = [
    ...
    },
    {
      ...
      content: m(FlamegraphView, {
        ...
        // Diff metrics only work when the baseline lives in the same
        // engine as the primary — cross-engine SQL JOINs aren't possible
        // across separate workers. For cross-trace baselines we fall
        // back to the single-engine current view.
        baseline:
          diffActive && getActiveBaseline()?.trace.engine === engine
            ? {
                upid: getActiveBaseline()!.dump.upid,
                ts: Time.fromRaw(getActiveBaseline()!.dump.ts),
              }
            : undefined,
      }),
    },
