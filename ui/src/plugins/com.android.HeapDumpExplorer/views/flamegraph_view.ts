...
// reflects movement magnitude; `color_hint` (HSL) encodes direction —
// red for grew / NEW, blue for shrank, gray for unchanged. Tooltip shows
// baseline/current/delta as aggregatable properties.
//
...
// per-process (different upids get disjoint id spaces) and not even
// always shared across dumps of one upid. Hashing the path of class
// names + heap_type makes the join key stable across processes and
// across dumps within one trace — so the same fixture works whether
// the user is comparing two snapshots of one process or two processes.
function buildDiffMetric(
  ...
  const statement = `
    ...
    SELECT
      ...
      CASE
        WHEN s.m IS NULL OR s.m = 0 THEN '#cccccc'
        WHEN j.is_new = 1 THEN '#c62828'
        WHEN j.delta_${dim} = 0 THEN '#cccccc'
        WHEN j.delta_${dim} > 0
          THEN printf('hsl(0, %d%%, %d%%)',
            cast(round(45.0 + 45.0 * abs(j.delta_${dim}) * 1.0 / s.m) AS INT),
            cast(round(75.0 - 25.0 * abs(j.delta_${dim}) * 1.0 / s.m) AS INT))
        ELSE printf('hsl(220, %d%%, %d%%)',
            cast(round(45.0 + 45.0 * abs(j.delta_${dim}) * 1.0 / s.m) AS INT),
            cast(round(75.0 - 25.0 * abs(j.delta_${dim}) * 1.0 / s.m) AS INT))
      END AS color_hint
